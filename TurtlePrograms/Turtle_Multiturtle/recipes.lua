...
recipe_outputMult = 1
recipe_inputMult = 1
woods = { "minecraft:oak_log", "minecraft:spruce_log", "minecraft:birch_log", "minecraft:jungle_log",
	"minecraft:acacia_log", "minecraft:dark_oak_log", "minecraft:crimson_log", "minecraft:warped_log",
	"myrtrees:rubberwood_log" }
planks = { "minecraft:oak_planks", "minecraft:spruce_planks", "minecraft:birch_planks", "minecraft:jungle_planks",
			...
			for y = 1, 3 do
				for x = 1, 3 do
					if turtle.getItemDetail() ~= nil and has_value(recipe_rec[y][x], turtle.getItemDetail().name) then
						if verbose then
							log("Moving " .. (recipe_count / recipe_outputMult) ..
								" items from slot" .. i .. " to slot " .. craftIndexToInvIndex(y, x))
						end
...
function recalculateItemsNeeded()
	...
	if recipe_count > recipe_maxCount then
		log("Warning: Recipe set for " ..
			recipe_count .. " of " .. recipe_name .. ", but maximum craftable in 1 batch is " .. recipe_maxCount .. "!")
	end
end
