...
function generateObtainmentPlanForItem(itemname, totalItemCount, necessaryForStepAbove)
    ...
    plan.preconditions = concatenateTables(plan.preconditions,
        generateObtainmentPlanForList(copyTable(recipes_itemsNeeded), plan.totalActionCount))
    return plan
end
...
function addItemsToPlanAndCalculateAvailableStepsHelper(plan, items)
    ...
    for _, substep in pairs(plan.preconditions) do
        maxPossible = math.min(maxPossible, math.floor(substep.itemsDone * substep.outMult /
            substep.necessaryForStepAbove))
    end
...
function addActionsDone(plan, addCount)
    ...
    for _, subPlan in pairs(plan.preconditions) do
        --log("Subplans:")
        --log(subPlan)
        local addCount = math.min(subPlan.totalActionCount * subPlan.outMult - subPlan.itemsDone,
            subPlan.necessaryForStepAbove * addCount)
