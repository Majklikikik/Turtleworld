...
function getmissingItems()
	...
	for item, _ in pairs(itemsWanted) do
		...
		copyItemsWanted[item] = itemsWanted[item]

		local alreadyInInventory = countOf(item)
		if alreadyInInventory > 0 then
			copyItemsWanted[item] = copyItemsWanted[item] - alreadyInInventory
			if copyItemsWanted[item] <= 0 then
				copyItemsWanted[item] = nil
			end
		end
	end
	...
	for i = chests["count"], 1, -1 do
		...
		for j = 8, 1, -1 do
			if chests[i].items[j] ~= nil then
				local c = copyItemsWanted[chests[i].items[j].name]
				if c ~= nil then
					toGet[i][j] = math.min(c, chests[i].items[j].count)
					copyItemsWanted[chests[i].items[j].name] = math.max(0, c - chests[i].items[j].count)
					if copyItemsWanted[chests[i].items[j].name] == 0 then copyItemsWanted[chests[i].items[j].name] = nil end
				end
			end
		end
		...
		if tableSize(copyItemsWanted) == 0 then
			break
		end
	end


	if not isEmpty(copyItemsWanted) then
